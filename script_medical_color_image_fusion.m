% Code for multimodal medical functional anatomical image fusion (PET-MRI)
...
%% multimodal inputs
folder_name = 'Medical_Color';
i1 = (imread([folder_name '\J6.png'])); % anatomical image 
if size(i1,3)>1
   i1 = single(rgb2gray(i1))/255;
else
   i1 = single(i1)/255; 
end

i2_rgb = single( (imread([folder_name '\I6.png']))  )/255; % functional image
i2_ycbcr = rgb2ycbcr(i2_rgb);
i2 = i2_ycbcr(:,:,1); % y layer (greyscale)
...
load('dicts\D0_med_color.mat')
load('dicts\C0_med_color.mat')
...
%% Fusion
IF_grey = fuse_grey(D,C,X,Gamma,i1_low,i2_low,0.8);
toc
IF_ycbcr(:,:,1) = IF_grey;
IF_ycbcr(:,:,2:3) = i2_ycbcr(:,:,2:3);
IF = ycbcr2rgb(IF_ycbcr);

imwrite(uint8(IF*255),'Results\med_colr_result.png')

% D0 = D; save('dicts\D0_med_color_new.mat','D0');
% C0 = C; save('dicts\C0_med_color_new.mat','C0');

figure(345)
subplot 131
imshow(i1,[0 1]),xlabel('i1')
subplot 132
imshow(i2_rgb,[0 1]),xlabel('i2')
subplot 133
